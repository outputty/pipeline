...
- f4ec5e2: Every runner is faster, with the same public API and outputs.
...
- c256856: `HttpPipeline`, `ClusterHttpPipeline` and `EventEmitterPipeline` move off the root entry to
...
- b8ff2d7: BREAKING: `.buffer(size)` on an ALREADY-BOUND pipeline now refuses a size that is not a whole number
...
- 138fa35: BREAKING: `new Pipeline({ chunkSize })` now refuses a value that is not a whole number of at least 1,
...
- 8917579: BREAKING: `Drainable<T>` no longer carries `items`, so `pipeline.drainable(input).items()` is gone.
...
- f808749: `EventEmitterPipeline`'s events read as the route the chain was built along, and the chain's own
...
- 93761d0: `HttpPipeline` accepts `options.client`: how a dispatched chunk reaches the other instance. It takes
...
- 4d63615: `ConcurrentPipeline`/`HttpPipeline`/`ClusterPipeline` pay less to run a fully synchronous chain over
...
- 6340f9a: `Transformer.filter()`'s synchronous, no-row-handler branch is now one pass over the chunk instead
