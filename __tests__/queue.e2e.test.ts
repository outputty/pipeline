...
describe("#123 queue(n) prefetches ahead of the consumer (Done-when 1)", () => {
  it("prints [6, 8, 10], unchanged from buffer(2).transform(...) alone", async () => {
    const out = await new Pipeline<number>()
      .buffer(2)
      .queue(3)
      .transform((t: Transformer<number, number, "async">) =>
...
describe("#123 queue(n) overlaps production and consumption (Done-when 2)", () => {
  it("runs measurably below the fully-serial baseline over the same delays", async () => {
...
describe("#123 queue(n) starts pulling only on the first consumer pull (Done-when 3)", () => {
  it("pulls zero items with no terminal call, then exactly capacity on the first pull", async () => {
    let pulls = 0;
    ...
    const chain = new Pipeline<number>().buffer(1).queue(3);
...
describe("#123 queue(n) closes the source on an early stop, on both engines (Done-when 4)", () => {
  it("closes a sync generator when .first(1) stops a chain ending in .queue()", async () => {
...
describe("#123 queue(n) surfaces a mid-stream throw to the consumer's next pull (Done-when 5)", () => {
  it("rejects the terminal call with the source's own error, consuming every chunk", async () => {
  ...
  it("leaves zero unhandledRejection events when a consumer stops before the throw is pulled", async () => {
...
describe("#123 queue(n) composes with ConcurrentPipeline.reduce()'s own partitioning (Done-when 6)", () => {
  it("returns two correct partition sums, no deadlock or starvation", async () => {
...
describe("#123 queue(n) refuses an invalid capacity, matching buffer()'s own wording (Done-when 7)", () => {
  it("throws on 0, a non-integer, and a negative capacity", () => {
