/**
 * codec.e2e.test.ts - #209's Done-when cases. 1-4 are `it.fails` (true only once the encoded-chunk
...
describe("#209 dispatched replies stay encoded between two stages (Done-when 2)", () => {
  it.fails("primary decode 8 encode 8 against base's own 16/16", async () => {
...
describe("#209 a dispatched reduce forwards encoded chunks too (Done-when 4)", () => {
  it.fails("output [73], the primary decodes only for the terminal .toArray()", async () => {
    const worker = makeWorker((t) => {
      const reduced = t
