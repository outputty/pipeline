...
export class ErrorHandler<In, U = void> {
   ...
   * replace or drop the failing chunk. `Transformer.onError()`'s own callers ignore it instead -
   * `chunkErrorReporter`, called by `ConcurrentPipeline.apply()`'s wrapped `work` immediately for a
   * dispatched stage, and by `Transformer.process()`'s own outer catch for a local one (#40); and
   * `process()`'s fallback `handle([], …)` call for the one failure that never reaches a chunk loop
   * at all (the upstream `AsyncIterable` itself rejecting, before any chunk is pulled) - `.onError()`
   * is a notification hook on every one of these paths, never a recovery path.
   *
