/**
 * Two classes, one file (#201): `ClusterHttpPipeline` (#17) — each chunk of a stage dispatched to
 * another PROCESS on the same machine over HTTP, via `node:cluster`. Reuses `HttpPipeline`'s own
 * dispatch and `.fetch()` wholesale - a cluster worker is just another `HttpPipeline` instance,
 * reached at `http://localhost:<bootstrapped port>`; `routePath()` changes to route several
 * pipeline definitions through the ONE server every worker runs (`/pipeline/<i>/<verb>/<n>`), and
 ...
 *
 * `ClusterPipeline` (#201) — the SAME "each chunk dispatched to another process" shape, over
 * `WebSocketPipeline`'s own multiplexed `ws+unix:` wire instead: each worker binds its own UNIQUE
 * unix socket path (never a shared port - a WebSocket connection is persistent, so `#201`'s own
 * Done-when 3 needs one distinct target per worker to count one connection each), and dispatch
 * round-robins across the bootstrapped set (`WsWorkerSet.enter()`). Mirrors `WorkerSet`'s own shape
 * throughout - `register()`/`claimIndex()`/`lookup()`/`bootstrap()`/`enter()`/`kill()` all carry the
 * identical role, one seam (a port vs. a set of socket paths) apart. `ClusterHttpPipeline` survives
 * unchanged under its own name for a caller who wants the old, unchanged HTTP/TCP transport;
 * `ClusterPipeline` is the class name every existing caller already imports (#201's own Constraints:
 * BREAKING, no deprecation period, no code change required).
 *
 * Both bring their own workers up lazily, on the first chunk actually dispatched - `stageWork()`
 * itself still runs at BUILD time (`ConcurrentPipeline.apply()` calls it synchronously), but the
 * bootstrap lives inside the closure it RETURNS, which only ever runs when a terminal op drains the
 * pipeline. A pipeline built and never drained (case 7's own `.constructor.name` check) never forks.
 */
 ...
 * `socketPath` - the WS-wire counterpart of `isReadyMessage` (above), same reason.
 *
 ...
 * once, on both the primary and every worker, mirroring `workerSet` (above). */
