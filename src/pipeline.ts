...
/** Builds a plain async-iterable from an async generator function - the
...
/** `EMPTY_CHUNKS`, cast to the chunk type a call site actually needs - `EMPTY_CHUNKS` itself stays
 * typed `never[]` so a caller reading it directly still sees "genuinely empty", not "empty of some
 * particular type".
 *
...
/** Whether `data` is itself async-iterable, rather than merely iterable - the one test `.from()`'s
 * own Mode decision and `toAsyncIterable()` below both need. */
 ...
 * source never goes through this. */
 ...
 * (`toArray`/`first`/…) and async iteration both read the SAME persisted chunk stream.
 */
export type PipelineSource<T> = AsyncIterable<T> | Iterable<T>;

/**
 * A `Pipeline` whose Mode is not tracked - what the copy-on-write seam `createPipeline()` and
...
/**
 * A chain a wrapping class can adopt: source-less, so its stages are still recorded calls to
 ...
 *
 * Any Mode, deliberately: refusing an `"unset"` chain here made wrapping a source-less pipeline a
 * compile error rather than a runtime one, but also refused a chain holding one async callback -
 * `new ConcurrentPipeline(asyncChain, {…})` was a type error on the very class that forces
 * `"async"` and exists for exactly that chain. Boundness is `adopt()`'s check, at runtime, where
 * the runtime fact `_bound` actually lives.
 */
 ...
 *
 * Everything else a pipeline carries is `PipelineState` below.
 */
export interface PipelineOptions {
  /**
   * An already-built context manager, for THIS process. Takes precedence over `contextFactory` -
   * a caller who already holds the instance they want (the orchestrating process, typically)
   * passes it here; a process that must build its OWN instance (a `ClusterPipeline` worker,
   * re-executing the same entry module with no way to receive an already-built instance across the
   * process boundary) uses `contextFactory` instead.
   */
   ...
   * the SAME instance the constructor built, never a second one. `context` and `contextFactory`
   * together is not an error: the instance serves this process, the factory serves every other one.
   */
  contextFactory?: () => IContextManager;
}

/**
 * Everything a copy-on-write call carries from one instance to the next - internal, and declared
 * apart from `PipelineOptions` so the exported surface is the two knobs above.
 *
 * `carriedOptions()` returns this whole shape rather than naming fields at each call site, which is
 * what stops a knob being dropped silently. `adopt()` and `emptyOfOwnClass()` still name their
 * fields, deliberately - one excludes what a wrapped chain must not inherit, the other resets an
 * arm to blank.
 */
export interface PipelineState {
  /** an already-cut chunk stream to seed `_chunks` with directly, bypassing the constructor's own
   * default cut - the copy-on-write path every method below (`.apply()`, `.buffer()`, `.context()`)
   * uses via `createPipeline()`. */
  chunks?: AsyncIterable<unknown[]>;
  /** the pre-buffer ITEM view `.buffer()` recuts from on a second, back-to-back call - `null` once
   * a real stage has consumed `chunks` (`.apply()` sets it), so a LATER `.buffer()` falls back to
   * flattening whatever that stage actually produced instead. */
  preBufferItems?: AsyncIterable<unknown> | null;
  /** the chain of chunk-wise transforms accumulated via `.apply()`/`.transform()` - `HttpPipeline`'s
   * own `.fetch()` looks a stage up by index here to serve a dispatched request. */
  chunkTransforms?: ChunkTransform[];
  /** every reduce stage registered via `.reduce()`, keyed by its index in the SAME shared space
   * `chunkTransforms` uses - `HttpPipeline`'s own `.fetch()` looks a stage up here to serve a
   * `/reduce/<n>` request. */
  reduceStages?: Map<number, ReduceStage>;
  /** The RUN handler, registered via `.onError()` - position-DEPENDENT, unlike `Transformer`'s own
   * row handler: only a stage applied AFTER `.onError()` sees it, since it reaches a chunk failure
   * only through that stage's own dispatch (`Pipeline.apply()`/`ConcurrentPipeline.apply()`).
   * Carried forward by every copy-on-write call the same way `context`/`chunkTransforms` are. */
  runHandler?: PipelineErrorHandler;
  /** the RUNTIME half of `PipelineMode` - which engine this pipeline's own terminal ops read. Set
   * by `.from()` from the source's shape and the class's own `SourcePolicy`, and carried forward by
   * every copy-on-write call. */
  mode?: PipelineMode;
  /** the SYNC chunk stream a `"sync"`-Mode pipeline reads, whose individual chunks may still be
   * pending once a stage's callback returned a thenable. `null` on an `"async"` chain, where
   * `chunks` carries the stream instead. */
  syncChunks?: MaybeAsyncChunks<unknown> | null;
  /** `preBufferItems`' sync counterpart - the raw item view a back-to-back `.buffer()` recuts from
   * on a `"sync"` chain. */
  syncPreBufferItems?: Iterable<unknown> | null;
  /** the chunk boundary `.buffer(size)` last declared, carried so a `.buffer()` called BEFORE
   * `.from()` still decides the source's own cut. */
  chunkSize?: number;
  /** every stage composed while the pipeline had no source, in order, replayed by `.from()` once an
   * input arrives. */
  pendingStages?: PendingStage[];
  /** the route prefix an arm's own stages address themselves under, `/branch/<i>/<name>`. Empty on
   * a chain's own stages. Without it an arm's stage 0 collided with the parent's stage 0 on the
   * worker, which served the parent's transform for both. */
  routeTrail?: string;
  /** every `.branch()` stage's own arms, keyed by the branch's index in the shared stage space -
   * the registry a serving side walks to resolve a `/branch/<i>/<name>/` trail. Not intended for
   * direct external use. */
  branchStages?: Map<number, BranchArm<unknown>[]>;
  /** whether `context` was invented by a `Pipeline` rather than named by the caller. A
   * default-built manager belongs to one run, so a reusable chain gets a fresh one per call; a
  ...
  /** whether an input has been bound to this chain. A RUNTIME fact, kept apart from `mode`, which
   * is a TYPE fact about what the chain produces: a callable chain is `"unset"` for its whole life
   * and becomes bound only for the duration of one call. */
  bound?: boolean;
}
 ...
 * if ever invoked as one. This is the fail-loud guard for a stale route: there is no separate
 * replay path left that a violations list would otherwise need to protect against. */
...
/**
 * A chain over `T`, holding its input TYPE and no data. Nothing runs until the pipeline is CALLED
 * and a terminal on the `PipelineResult` (`toArray`, `first`, iteration, …) pulls: `.apply()`/
 * `.transform()` compose transformers, chunking is handled for you, and a chunk's items flow to
 * the next stage as a group. The whole chain shares one context manager, so a context-aware
 * transformer can read and write state across stages — `.context()` itself still returns a NEW
 * `Pipeline` (copy-on-write, like `.apply()`/`.transform()`/`.buffer()`), carrying the SAME context
 * manager forward, mutated in place — a caller's own `IContextManager` class is never copied into
 * a fresh `SimpleContextManager` and discarded.
 *
 * `new Pipeline<number>().transform((t) => t.map((x) => x * 2))([1, 2, 3]).toArray()` → `[2, 4, 6]`,
 * with no `await`: every callback here is synchronous.
 */
export interface Pipeline<T, M extends PipelineMode = "unset", In = T> {
  /** Binds an async input. Widens the chain's own Mode to `"async"` regardless of `M`. */
  (input: AsyncIterable<In>): PipelineResult<T, "async">;
  /** Binds a sync input. The result stays `"sync"` unless the chain itself was already widened by
   * an async callback somewhere in the composed chain. */
  (input: Iterable<In>): PipelineResult<T, M extends "async" ? "async" : "sync">;
}

/** The class backing the `Pipeline` interface above - its call signatures document what calling an
 * instance does; this declaration carries every method. */
export class Pipeline<T, M extends PipelineMode = "unset", In = T> {
  // Protected, not private: a dispatching subclass's own overridden `createPipeline()` (below)
  // reads these to carry them into the next instance the same way this base implementation does -
  // `private` would put them out of reach from `src/pipelines/`.
  //
  // Each carries a definite-assignment `!` because the constructor assigns them onto the FUNCTION
  // it returns rather than onto `this` (see the constructor). TypeScript checks initialization
  ...
  constructor(options?: PipelineConstructorOptions) {
    // A constructor that RETURNS a function is what makes an instance callable. Two halves, both
    // load-bearing:
    //
    ...
    // would throw `EvalError: Code generation from strings disallowed for this context` under
    // `node --disallow-code-generation-from-strings`, and on a CSP page or a Cloudflare Worker.
    // Reparenting the prototype costs nothing and runs everywhere.
    //
    ...
    // at runtime, so a JS consumer reaches `.bind` too - `score.call(null, [1,2,3])` returns a
    // `PipelineResult`, `score.apply(null, [[1,2,3]])` reaches `Pipeline.apply()` instead, and
    // `score.bind(null)` returns a `Pipeline` bound to `null` that throws when called. Only `.call`
    // is unaffected. A consumer that invokes callbacks via `fn.apply(ctx, args)` or `fn.bind(ctx)`
    // needs a wrapper: `(input) => score(input)`.
    const self = ((input: PipelineSource<unknown>) => {
      ...
      // calling one silently dropped every earlier stage. Refused rather than half-honoured; the
      // state disappears entirely once `.from()` does, at which point every pipeline is callable
      // and none is bound.
      if (self._bound) {
        throw new Error(
    ...
    // once per process, not once per stage or per request.
  ...
  /**
   * Names the source, and with it the engine the whole chain runs on - an `Iterable` makes the
   ...
   *
   * An ordinary method, not a constructor parameter: a constructor overload cannot vary its class's
   * own generic return, so a constructor could never infer the Mode from the source's own shape.
   *
   * @example
  ...
  // A receiver already widened to `"async"` stays async whatever the source's own shape:
  // `.onError()` and `.context()` are both callable BEFORE `.from()`, so an async run handler
  // registered there would otherwise have its widening discarded here. `"unset"` is the ordinary
  ...
  /**
   * The shared body behind every class's own `.from()`. `policy` is what a dispatching subclass
   * overrides via `sourcePolicy()` below - it never re-implements this cutting and bookkeeping, so
   * a change to how a source becomes chunks lands once rather than four times.
   *
   * A `"sync"` source is cut by `buildSyncChunkGenerator` and never becomes an async iterator at
   * all - that per-item conversion, not the per-chunk `Promise.all`, is most of the cost a fully
   * synchronous chain would otherwise pay for zero transform stages.
   */
    ...
    if (mode === "sync") {
      ...
      return this.replayPending(
        this.createPipeline<U>(emptyChunks<U>(), {
          ...
          // both callable on a source-less pipeline, and dropping them here would make a registered
          // run handler silently never fire.
          ...this.carriedOptions(),
          mode,
          pendingStages: [],
  ...
  /**
   * Runs every stage this pipeline recorded while it had no source, in order, against `bound`.
   *
  ...
  /**
   * What THIS class does to a source's own shape - the runtime half of `SourcePolicy`. The base
   * keeps the shape; every dispatching class overrides this to `"async"`, which is the ONE line
   ...
   * hard-coded `new Pipeline<U>` — the ONE seam every copy-on-write method below (`.apply()`,
   * `.context()`, `.buffer()`) goes through, so a subclass built on `Pipeline` survives its own
   * `.transform()` chain instead of silently decaying to a plain `Pipeline`.
   *
   * `chunks` is an ALREADY-CUT stream - this method never cuts one of its own, it only threads the
   * caller's chunk stream (plus context/chunkTransforms/preBufferItems) into a fresh instance of
   * THIS pipeline's own class.
   *
   ...
   * method itself - `this.constructor` alone only reproduces knobs `PipelineOptions` itself already
   * carries. This base implementation is correct for `Pipeline` itself and for any subclass whose
   * constructor takes nothing beyond `(source, options)`.
   *
   * `R` is the caller's own return type, the same seam `defer()` above uses and for the same
   ...
   * already carries - `createPipeline()` above spreads this onto every copy-on-write instance it
   * builds, which is the one seam a subclass needs to override rather than `createPipeline()` as a
   * whole. The base has none. `ConcurrentPipeline` overrides it to return `{ maxConcurrency,
   * ordered }`; `HttpPipeline`/`ClusterPipeline`/`EventEmitterPipeline` each extend their PARENT's
   * own override with `{ ...super.carriedKnobs(), <their own field(s)> }`.
   *
  ...
  /** Every knob a copy-on-write call carries into the next instance - named once here rather than
   * repeated field by field at each call site, so a knob added later reaches all of them. `chunks`
   * is passed separately, since each caller supplies its own. */
  protected carriedOptions(): PipelineConstructorOptions {
    return {
      ...
      // chain's `.branch()` would land in every ancestor's and sibling's registry, and
      // `branchIndex` (derived from `.size`) would then depend on which sibling was declared first.
      branchStages: new Map(this._branchStages),
    };
  }

  /** The pre-buffer reset every REAL stage needs, once it has consumed the chunk stream - nothing
   * is left to recut a back-to-back `.buffer()` from except that stage's own output, so both views
   * null out. Named once here rather than repeated at each dispatched call site - a dispatching
   * subclass has no sync chunk stream of its own, so nulling `syncPreBufferItems` there is a no-op,
   ...
   * one reusable chain never see each other's writes. */
  protected contextForRun(): IContextManager {
    ...
    // declares a default every run starts from, and an empty manager here would drop it, so a
    // stage reading `ctx.getOrDefault("multiplier", 1)` fell back to `1`. Seeded and separate is
    // what keeps `.context()` working while two calls still never see each other's writes.
    return new SimpleContextManager(this._context.toDict());
  }

  /**
   * Records a stage instead of running it, on a pipeline that has no source yet.
   *
   ...
   *
   * `R` is the caller's own return type - `createPipeline()` only ever produces the loosely typed
   * `AnyPipeline<U>`, so `R` moves the caller's own type assertion inside `defer()` itself: a
   * caller writes `this.defer<U, Pipeline<U, JoinMode<M, M2>, In>>(...)` and gets the precise type
   * back with no trailing cast of its own.
   *
   * @example
   ...
   * arrives, which is every pipeline built the callable way. */
  protected isDeferred(): boolean {
    return !this._bound;
  }

  /**
   * This pipeline's stage registries - the two maps a SERVING side reads to answer `/transform/<n>`
   * and `/reduce/<n>`.
   *
   ...
   * memoised - the empty source produces no chunks, so nothing runs; only the registries are the
   * point.
   *
   * @example
  ...
  /**
   * The registries a `/branch/<i>/<name>/` trail addresses: the ARM's own, not this pipeline's. The
   * worker runs the same entry module, so its `.branch()` call built the same arms in the same
   * order - resolving the trail rebuilds the arm's pipeline and reads what it registered.
   *
   * @example
  ...
  /**
   * The options that reproduce `pipeline`'s chain on ANOTHER class - what a wrapping class's
   ...
   * call takes: `(pipeline, options)` adopts a chain built elsewhere, `(options)` builds an empty
   * one. Written once here so `ConcurrentPipeline`, `HttpPipeline` and `ClusterPipeline` cannot
   * drift on which of the two they accept, or on how a wrapped chain is carried in.
   *
   * @example
  ...
  /** This pipeline's CHUNKS as an async stream, whichever engine it runs on - a `"sync"` pipeline
   * leaves `_chunks` empty and carries `_syncChunks` instead, so this is the one place that
   * difference is resolved. `asyncItems()` and `PipelineResult.chunks()` both read it. */
  ...
  /** Whether this pipeline runs on the synchronous engine - what `reduce()`/`apply()`/`buffer()`/
   * `chunkStream()`/`syncChunkStream()` all ask before choosing between the sync and the async
   * engine. */
  ...
  /** This pipeline's items, as one stream, whichever engine it runs on - the seam the async
  ...
  /** This pipeline's own context manager. Read-only: writes go through `.context()`. */
   ...
   * `ctx`, so a manager that refuses an unknown key (a sealed manager) throws HERE, propagating to
   * the caller, instead of being silently bypassed by a copy step that never called its `.set()` at
   * all. A caller still holding the pre-`.context()` reference sees every later write too - both
   * references alias the SAME manager. The same aliasing applies to TWO SIBLINGS built off one
   * shared base: `const a = base.context({m:"a"}); const b = base.context({m:"b"})` leaves `a`, `b`
   * and `base` all reading the ONE manager `base` started with, so `b`'s write is the value every
   * one of them sees - forking into independently-configured branches needs a caller-supplied
   * manager per branch, never two `.context()` calls off the same parent. Per-key, not
   * transactional: `ctx`'s entries are set one at a time in `Object.entries()` order, so a manager
   * that throws partway through (a sealed manager rejecting one key among several) leaves every
   * EARLIER key's write already applied - the same partial-application a caller looping `.set()`
   * calls by hand would get, never rolled back.
   *
   ...
   *   manager forward. Typed `this`, not `Pipeline<T>` - `T` never changes here, so a dispatching
   *   subclass's own `.local(build)` region still typechecks after a `.context()` call, the same as
   *   it would directly off the constructor.
   *
   * @example
   * A manager that records every key written, handed in at construction:
   * `new Pipeline([1, 2], { context: mine }).context({ multiplier: 10 }).transform((t) =>
   * t.map((x, ctx) => (ctx.set("k", x), x))).toArray()` then `mine.writes` →
   * `["multiplier", "k", "k"]` - `mine` itself received every write.
   */
  ...
  /**
   * Registers the RUN handler - position-DEPENDENT, unlike `Transformer.onError()`'s own row
  ...
  onError(handler: PipelineErrorHandler): this | Pipeline<T, "async", In> {
    // An ASYNC handler widens the chain, the same rule `.tap()` follows. `PipelineErrorHandler`
    // declares a bare `void` return, which accepts an `async` function silently, so without the
    // overload above the chain would keep its `"sync"` type while `dropOrRethrow` deferred on the
    // handler's own promise - a chain typed `number[]` handing back a pending `Promise` the moment
    // an error actually fired.
    //
    ...
    // Defers like a stage, because it IS positional: `.onError()` covers only stages applied AFTER
    // it, and setting `runHandler` on the deferred pipeline instead would make it cover the whole
    // chain - a handler written after `.transform()` would silently start catching that transform.
    if (this.isDeferred()) {
   ...
   * runs it directly over the pipeline's own persisted chunk stream, no cut here: chunking is
   * never this method's decision, only `.buffer()`'s.
   *
   * `transformer.runnable()` is what carries the transformer's own row handler into
   * `_chunkTransforms` - the SAME function `process()` drives, so a dispatched stage on
   * `HttpPipeline`/`ClusterPipeline` gets row recovery too, off the identical entry a worker's own
   * copy of this chain built. `this._runHandler` (`.onError()`) is threaded through as `process()`'s
   * own third argument, reaching `runSequentially`'s per-chunk catch.
   *
  ...
  apply<U, M2 extends "sync" | "async">(
    ...
    // No source yet: record the call and replay it when one arrives. This is the ordinary case
    // for a callable pipeline, where the chain is composed before any data exists.
    if (this.isDeferred()) {
    ...
    // A `"sync"` chain maps its chunks through the SAME `runnable` an async chain does, but through
    // a sync generator: no `Promise` is created unless the transformer itself returns one, and
    // that chunk then travels the stream as a pending value the terminal op settles.
    if (this.isSync()) {
   ...
   *
   * Composable before an input exists: a source-less `.transform()` records the stage; calling the
   * pipeline replays it. What still refuses is DRAINING without an input, which the
   * `Pipeline`/`PipelineResult` split makes a type error rather than a runtime one.
   */
  ...
  /**
   * The chunk boundary - explicit, opt-in. Every later `.transform()`/`.apply()` sees these chunks
   * unchanged until another `.buffer()` call declares a new one. `size` cuts by count; `fn:
   * BufferFunction<T>` decides the boundary per item instead - a `T[]` pending array the framework
   * owns, folded through it item by item. `fn`'s own `emit()` takes no value: it flushes whatever is
   * pending and resets it to `[]`; returning a value appends it to the (possibly just-reset) pending
   * array, returning `DROP` skips the item entirely. `.buffer(size)` is this same engine configured
   * with an identity `fn` and a framework-side auto-flush at `pending.length >= size` - one engine,
   * not two.
   *
   * Recuts from `_preBufferItems` (the raw item stream) when it is still set - nothing has consumed
   * `_chunks` since the last cut, so a run of `.buffer()` calls with nothing between them collapses
   * to only the LAST one ever actually applied, never stacking a redundant flatten-then-recut on top
   * of an intermediate cut nobody asked to see. Once a real stage (`.apply()`) has run,
   * `_preBufferItems` is `null` and this flattens `_chunks` itself first - a genuine re-chunk of
   * that stage's own output.
   *
   * Typed `this` - `T` never changes here either, so this stays chainable on a dispatching subclass
   * without losing its own `.local(build)` overload. `.buffer(fn)` widens Mode to `"async"` when
   * `fn` returns a `Promise`, the same rule `.transform()`/`.reduce()` already follow.
   *
   * @example
  ...
  buffer(sizeOrFn: number | BufferFunction<T>): this | Pipeline<T, "async", In> {
    ...
    // the same deferral `.apply()`/`.reduce()`/`.local()` use, for the same reason.
    //
    // Recording only the size, rather than the call, loses the call's position: `fromSource` would
    // then apply it to the SOURCE cut, so a `.buffer()` written after a stage would take effect
    // before it. `chunkSize` is still carried alongside a NUMERIC `.buffer()`, because a `.buffer()`
    // written BEFORE any stage must also cut the source itself, which is what it now does by
    // replaying against a pipeline whose source is already cut at that size. `BufferFunction` has
    // no equivalent knob (`_chunkSize` is a plain `number`), so a deferred `.buffer(fn)` leaves the
    // SOURCE's own first cut at the default and relies entirely on its own replay's re-cut from
    // `_preBufferItems` - the same final chunking either way, since that re-cut always runs
    // regardless of what the first cut used.
    if (this.isDeferred()) {
      // Validated HERE as well as in `sizeReduceFunction` (below), because a deferred `.buffer()`
      // only records the call - every chain is source-less by default now, so that is the ordinary
      // path, and `new Pipeline<number>().buffer(0)` should fail with a message that names
      // `.buffer()`, not a generic error at a later drain. Non-integer refused as well as `< 1`:
      // the two cutting paths round it differently, and one call over the same data would
      // otherwise produce two different chunk boundaries with no error either way.
    ...
    // "One engine, not two": both overloads fold items through the SAME `Reducer<T[], T>`-based
    // engine (`src/utils/reduce.ts`) - `sizeReduceFunction` configures it with an identity fn and a
    // framework-side auto-flush at `pending.length >= size`, `bufferReduceFunction` adapts a
    ...
    // The `"sync"` arm recuts with the sync chunker - going through the async one here would make
    // `.buffer()` alone widen a chain whose every callback is synchronous, which is exactly the
    // Mode/runtime divergence this file exists to keep in check.
    if (this.isSync()) {
      ...
      // `recutSyncChunks`'s own index-based re-slice untouched here - it operates on already-cut
      // arrays with no per-item decision to make. A `BufferFunction` folds each existing chunk
   ...
   * from `.buffer()`'s own already-cut stream from WHEN a downstream terminal asks for it.
  ...
  /**
   * Fold every chunk this pipeline produces into one or more values, sequential and in-process.
   * `Transformer.reduce` folds ONE chunk and keeps nothing between chunks; this folds EVERYTHING
   * the pipeline produces, the only place cross-chunk state lives. `ConcurrentPipeline` overrides
   * this to dispatch it instead - `.local(build)` is what keeps a reduce stage in-process on a
   * dispatching class now.
   *
   * A fold is order-dependent but not inherently asynchronous: over a `"sync"` chain a plain
  ...
  reduce<U>(fn: ReduceFunction<U, T>, initial: U): AnyPipeline<U> {
    // A reduce stage defers like any other - see `apply()`. Replaying it through this same method
    // is what keeps its own registry (`_reduceStages`) and its Mode rule identical either way,
    // rather than a deferred fold needing a second implementation.
    if (this.isDeferred()) {
    ...
    // Spread, never field by field: a hand-built option object here can drop a knob nobody
    // remembered to add. The overrides below are the fields this stage genuinely changes.
    const carried = {
      ...this.carriedOptions(),
      chunkTransforms,
      reduceStages,
   ...
   * context - a region, not a per-stage flag, so several consecutive stages that must stay in the
   * orchestrating process are written once instead of repeated on every one of them. Nothing
  ...
  local<U, M2 extends PipelineMode>(
    ...
    // A region defers whole - see `apply()`. Deferring the `.local()` CALL rather than its built
    // region is what keeps `build`'s own stages recorded against the region's real pipeline, which
    // is where they pin themselves in the orchestrating process.
    if (this.isDeferred()) {
      return this.defer<U, Pipeline<U, JoinMode<M, M2>, In>>((p) =>
        (p as unknown as Pipeline<T, M, In>).local(build),
      );
    }
    // Both option objects SPREAD their source pipeline's own carried knobs - see `reduce()` for
    // why: a hand-built list here can drop `bound`, in which case the region would defer instead
    // of running.
   ...
   * called on. Delegates to `Transformer.tap`, wrapped in `.local(build)` so the callback and its
   * context writes stay where the caller is even on `HttpPipeline`/`ClusterPipeline`, where a
   ...
   * `_chunkTransforms` already uses - `ConcurrentPipeline.reduce()`'s own override calls this too,
   * so both share one bookkeeping seam rather than two copies that could drift apart.
   *
   * @example
  ...
  /**
   * What a `PipelineResult` needs to drain this pipeline - the ONE seam the terminal ops read.
   *
   * A terminal op lives on the result of calling a pipeline, never on the pipeline itself - a
   * chain with no input has nothing to drain, which this split makes a type error rather than a
   * runtime one. This exposes the two views a result needs - the sync chunk stream where there is
   * one, and the item stream otherwise - so neither class has to reach into the other's fields.
   *
   * @example
  ...
  drainable(input: PipelineSource<In>): Drainable<T> {
    ...
    return {
      ...
      // THIS run's manager, which is a fresh one per call unless the caller named their own.
  ...
  /**
   * Routes items into named arms, each with its own pipeline of THIS class.
   *
  ...
  /**
   * A stage-less pipeline of THIS pipeline's own class, for one `.branch()` arm to build on.
   *
   * The class is the whole point: an arm built on `HttpPipeline` dispatches its stages the way the
   * parent's do, and `.local()` inside the arm's builder pins it here instead.
   *
   * @example
...
// Every `Pipeline` instance IS a function (see the constructor), so its prototype chain must reach
// `Function.prototype` - that is what makes `instanceof Function`, `.bind` and `.call` work on one.
// Done here, once, rather than via `class Pipeline extends Function`: `super()` on a `Function`
// subclass runs `CreateDynamicFunction`, which throws `EvalError: Code generation from strings
// disallowed for this context` wherever code generation is banned (a CSP page, a Cloudflare
